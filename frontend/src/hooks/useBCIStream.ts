...
export const useBCIStream = () => {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    ws.onopen = () => {
      console.log(`[BCI-WS] WebSocket connected to ${API_ENDPOINTS.BCI_WS}`);
      setStatus("connected");
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.samples) {
          ...
          console.error(`[BCI-WS] Server error:`, msg.error);
          setErrorMessage(msg.error);
          setIsStreaming(false);
        } else {
          console.warn(`[BCI-WS] Unknown message format:`, msg);
        }
      } catch (e) {
        console.error("[BCI-WS] Error parsing WebSocket message:", e);
      }
    };
    ws.onerror = (event) => {
      console.error(`[BCI-WS] WebSocket error:`, event);
    };
    ws.onclose = (event) => {
      console.warn(`[BCI-WS] WebSocket closed: code=${event.code} reason=${event.reason}`);
  ...
  const executeStreamAction = async (action: "start" | "stop") => {
    console.log(`[BCI] executeStreamAction called: action=${action}`);
    try {
      const endpoint =
        action === "start" ? API_ENDPOINTS.BCI_START : API_ENDPOINTS.BCI_STOP;
      console.log(`[BCI] Fetching ${endpoint}`);
      const response = await fetch(endpoint, { method: "POST" });
      console.log(`[BCI] Response: status=${response.status}`);

      // Handle 400 errors gracefully (e.g., trying to stop when not running)
      if (!response.ok && response.status === 400) {
        console.warn(`[BCI] Got 400, clearing error and stopping`);
        setErrorMessage(null);
        setIsStreaming(false);
        return;
      }
      ...
      console.log(`[BCI] Response data:`, data);

      if (data.error) {
        console.error(`[BCI] Server returned error: ${data.error}`);
        ...
        console.log(`[BCI] Stream started, waiting for WS samples...`);
        setStatus("connected");
        setErrorMessage(null);
        // isStreaming will be set to true when samples start arriving
      }
    } catch (e) {
      console.error(`[BCI] executeStreamAction failed:`, e);

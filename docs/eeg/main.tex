...
\item Two-stage LaBraM adaptation (head-only stage, then controlled unfreezing) with MCC-based checkpoint selection.
...
Run J & LaBraM 2-stage (head-only $\rightarrow$ unfreeze last 2; MCC-selected) & 0.5378 & Best LaBraM MCC=0.0753; persistent collapse tendency. \\
\bottomrule
\end{longtable}
...
\subsection{Two-Stage LaBraM Adaptation (Run J)}
The updated two-stage training run used:
\begin{itemize}
\item \textbf{Stage 1:} head-only training for 12 epochs.
\item \textbf{Stage 2:} unfreeze last 2 encoder blocks with low encoder learning rate.
\item \textbf{Selection rule:} MCC-based early stopping/checkpointing (binary task).
\end{itemize}

Observed outcomes:
\begin{itemize}
\item \textbf{Best MCC:} $0.0753$ at epoch 13 (immediately after stage transition).
\item \textbf{At best MCC checkpoint:} validation accuracy $0.5378$, balanced accuracy $0.5339$.
\item \textbf{Early-stop epoch:} 53.
\item \textbf{Behavior:} prediction histograms still oscillated between near-single-class states (e.g., $[450,0]$, $[0,450]$, and skewed mixtures).
\end{itemize}

Interpretation: the two-stage policy improved the \emph{selection signal} (MCC) and gave a reproducible best-MCC LaBraM checkpoint, but did not improve top-line validation accuracy beyond prior best LaBraM binary runs ($0.5378$).
...
\begin{ansbox}{3}
\textbf{RQ3 (intervention effectiveness)}: interventions behaved differently across regimes. For cross-subject FM adaptation, improvements remained incremental (best LaBraM binary validation accuracy 0.5378; full fine-tuning 0.5222) with persistent collapse dynamics. The two-stage LaBraM policy improved MCC-based selection quality (best MCC 0.0753) but did not raise headline validation accuracy beyond prior LaBraM best. For compact task-specific models, the EEGNetResidual baseline reached 0.5919 on the larger PhysioNet split. For within-user personalization, Muse calibration fine-tuning reached 0.9286 on a small holdout, showing that targeted user adaptation can materially improve practical control performance. Overall, intervention impact is therefore \emph{regime-dependent}: weak-to-moderate for current FM transfer, moderate for generic compact baselines, and strong for same-user calibration.
\end{ansbox}

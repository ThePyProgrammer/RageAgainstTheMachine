...
async def start_stream():
    ...
    try:
        logger.info("[/start] Start stream requested")
        ok, reason = streamer.start()
        logger.info("[/start] streamer.start() returned ok=%s reason=%s", ok, reason)
        if not ok:
...
async def stream_websocket(websocket: WebSocket):
    ...
    logger.info("[WS] New WebSocket client connecting")
    await websocket.accept()
    streamer.register_client(websocket)
    logger.info("[WS] Client registered. Total clients: %d", len(streamer.ws_broadcaster.clients))
